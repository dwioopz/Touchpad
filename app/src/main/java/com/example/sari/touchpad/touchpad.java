...
public class touchpad extends Activity {
    ...
    protected boolean connect(String to) {
        return connect(to, 0);
    }

    protected boolean connect(String to, boolean reconnect) {
        return connect(to, 0, reconnect);
    }

    protected boolean connect(final String to, int password) {
        return connect(to, password, false);
    }
    ...
    protected boolean connect(final String to, int password, boolean reconnect) {
        ...
        try {
            ...
            if (response[0] != 0x00) {
                ...
                // Jika password salah, akan diberi error
                if (response[0] != 0x01 || response[1] != 0x01)
                    throw new Exception(getString(R.string.error_connect));

                // Jika password sudah pernah digunakan, akan diberi error juga
    ...
    protected void findServers(Menu menu) throws Exception {
        // Melakukan ping broadcast untuk mendapatkan server
    ...
    public boolean onContextItemSelected(MenuItem item) {
        if (item.getGroupId() == SERVER_FOUND_ID) {
            // Menghubungkan ke item menu server
            ...
            // Mengijinkan user dan server saling terhubung
            AlertDialog.Builder alert = new AlertDialog.Builder(this);
